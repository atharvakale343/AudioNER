...
def main():
    AUDIO_TRANSCRIPTION_MODEL_URL = "http://127.0.0.1:5000/transcriptionmodel"  # The URL of the server
    NER_MODEL_URL = "http://127.0.0.1:5000/nermodel"

    # Audio TRANSCTIPTION
    client = MLClient(AUDIO_TRANSCRIPTION_MODEL_URL)  # Create an instance of the MLClient object
    ...
    audio_transcription_response = client.request(inputs, data_type)  # Send a request to the server
    ...
    for i in range(no_of_files):
        audio_ner_result = [{"file_path": f['file_path'], "result":r['result']} for f, r in zip(audio_transcription_response,ner_response)]
